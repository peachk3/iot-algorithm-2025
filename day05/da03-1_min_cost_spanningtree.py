...
G1.graph[CU][GS25] = 1; G1.graph[GS25][Seven11] = 1; G1.graph[CU][MiniStop] = 1
...
current = 0         # 시작 편의점
maxStore = current  # 최대 개수 편의저 번호(GS25)
    ...
    for vertex in range(gSize):
        if G1.graph[current][vertex] == 1:
            if vertex in visitedAry: # 방문한 적이 있는 편의점이면 탈락
                pass
            else:       # 방문한 적이 없는 편의점이면 다름 편의점으로 기정
                next = vertex
                break
