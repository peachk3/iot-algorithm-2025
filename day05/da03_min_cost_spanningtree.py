...
def printGraph(g): 
    ...
    for row in range(g.SIZE):
        print(nameAry[row], end='\t\t') # 행마다 정점이름 표시
...
while len(newAry) > (SIZE - 1): # 간선의 수가 (정점수 -1) 될때까지 반복
    ...
    saveWeight = newAry[index][0] # 현재 가중치 저장(복원 위해서)
    ...
    if startYN and endYN: # 둘 다 다른 간선이 있으므로 현재 간선 지워도 됨
        del(newAry[index])
    else: # 연결된 간선이 없으므로 지웠던 인접행렬 값을 다시 원상복귀
